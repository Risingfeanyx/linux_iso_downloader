#!/bin/bash
mkdir linux_downloader ; cd linux_downloader || exit
pause(){
...
show_menus() {
	...
	echo " 8. Kill Running Downloads:(NOTE: this will kill all instances of wget and blow away any in progress or downloaded ISOS)"
}
read_options(){
	local choice
	read -rp "Choose 1-8, CTRL-C to exit" choice
	case $choice in
		1) one ;;
		2) two ;;
		3) three ;;
